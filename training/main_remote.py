...
"""
リモート環境用メインスクリプト
- thing_project の学習結果を解析
- 全結果を transfer フォルダに保存
- タイムスタンプベースで複数実行に対応
"""
...
def run_analysis(project_dir: Path, transfer_dir: Path, run_id: str):
    ...
    try:
        ...
        if not config_path.exists():
            log_message(logger, "NG", f"Config 見つかりません: {config_path}")
            return False

        config = load_config(config_path)
        log_message(logger, "OK", "Config 読み込み完了")

        checkpoints_dir = project_dir / 'checkpoints'
        eval_results_dir = project_dir / 'eval_results'

        if not checkpoints_dir.exists():
            log_message(logger, "WARN", f"Checkpoints フォルダが見つかりません: {checkpoints_dir}")
        if not eval_results_dir.exists():
            log_message(logger, "WARN", f"Eval results フォルダが見つかりません: {eval_results_dir}")
        ...
        for phase_name, checkpoint_path in config['checkpoints'].items():
            try:
                log_message(logger, "INFO", f"処理中: {phase_name} ({checkpoint_path})")
        ...
        log_message(logger, "INFO", "報酬を正規化中...")
        if graph_gen.normalize_rewards():
            log_message(logger, "OK", "報酬正規化完了")
        else:
            log_message(logger, "WARN", "報酬正規化に警告あり")

        log_message(logger, "INFO", "学習曲線グラフを生成中...")
        if graph_gen.generate_learning_curves():
            log_message(logger, "OK", "学習曲線グラフ生成完了")
        else:
            log_message(logger, "WARN", "学習曲線グラフ生成に警告あり")

        log_message(logger, "INFO", "推論結果グラフを生成中...")
        if graph_gen.generate_inference_graphs():
        ...
        metadata = {
            ...
            "checkpoints_used": config['checkpoints'],
            "eval_configs": config['eval_configs'],
            "phase_scale_factors": {name: phase['scale_factors'] for name, phase in config['phases'].items()},
        ...
        from lib.inference_analyzer import InferenceAnalyzer
        analyzer = InferenceAnalyzer(eval_results_dir, config['eval_configs'])
        tracking_rates = analyzer.calculate_tracking_rates()
...
【追従率（Tracking Rate）】
  Lin Phase:   VX={tracking_rates.get('lin', {}).get('vx', 0):.1f}%  YAW={tracking_rates.get('lin', {}).get('yaw', 0):.1f}%
  Ang Phase:   VX={tracking_rates.get('ang', {}).get('vx', 0):.1f}%  YAW={tracking_rates.get('ang', {}).get('yaw', 0):.1f}%
  Alive Phase: VX={tracking_rates.get('alive', {}).get('vx', 0):.1f}%  YAW={tracking_rates.get('alive', {}).get('yaw', 0):.1f}%
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"LEAP Hand 学習結果解析システム (リモート版)")
    print(f"{'='*60}")
    print(f"実行ID: {run_id}")
