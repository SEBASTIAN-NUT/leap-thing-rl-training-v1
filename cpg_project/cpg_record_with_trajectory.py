...
def run_cpg(push_m=0.020, lift_m=0.030, freq_hz=1.0, duration=1.0, output_prefix="cpg_traj"):
    ...
    for step_i in range(nsteps):
        ...
        if renderer is not None:
            ...
            renderer.update_scene(d, camera=None)
            
            # Add trajectory lines to scene
            if len(trajectory_points) > 1:
                for i in range(len(trajectory_points) - 1):
                    ...
                    if renderer._scene.ngeom <= renderer._scene.maxgeom:
                        geom = renderer._scene.geoms[renderer._scene.ngeom - 1]
                        mujoco.mjv_makeConnector(geom, mujoco.mjtGeom.mjGEOM_LINE, 0.002,
                                               p1[0], p1[1], p1[2], p2[0], p2[1], p2[2])
                        geom.rgba[:] = [0.0, 0.5, 1.0, 1.0]
    ...
    # Also save trajectory CSV for reference
    if trajectory_points:
        csv_path = output_prefix + "_trajectory.csv"
        with open(csv_path, 'w', newline='') as f:
            w = csv.writer(f)
            w.writerow(['x_world', 'y_world', 'z_world'])
            for pt in trajectory_points:
                w.writerow([f"{pt[0]:.6f}", f"{pt[1]:.6f}", f"{pt[2]:.6f}"])
        print(f"Wrote {csv_path} ({len(trajectory_points)} points)")
...
if __name__ == "__main__":
    ...
    p.add_argument("--out_prefix", type=str, default="cpg_run_traj")
    import os
    os.chdir("..")
    args = p.parse_args()
    run_cpg(push_m=args.push, lift_m=args.lift, freq_hz=args.freq, duration=args.duration, output_prefix=args.out_prefix)