...
def domain_randomize(
    model: mjx.Model, rng: jax.Array, *, floor_geom_id: int, torso_body_id: int
):
    # NOTE: floor_geom_id/torso_body_id used to be hardcoded module-level
    # constants (FLOOR_GEOM_ID=0, TORSO_BODY_ID=1) copied from the duck
    # model. For the LEAP hand, geom id 0 is actually "palm_visual" (a
    # non-colliding visual mesh -- randomizing its friction has zero
    # physical effect), not the floor (real id: 25). Made these required
    # keyword args, resolved by name from the real mj_model in runner.py,
    # so this can never silently target the wrong geom/body again.
