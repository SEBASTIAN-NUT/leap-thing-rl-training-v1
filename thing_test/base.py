...
class OpenDuckMiniV2Env(mjx_env.MjxEnv):
    ...
    def __init__(
        ...
        # Bug fix: use actuator_trnid to get the connected joint name,
        # so this works even when actuator names differ from joint names (e.g. _act suffix in LEAP hand).
        self.actuator_names = [
            self._mj_model.jnt(self._mj_model.actuator_trnid[k, 0]).name
            for k in range(0, self._mj_model.nu)
        ]  # joint names for each actuator
        self.joint_names = [  # all joints (freejoint + actuated + backlash)
            self._mj_model.jnt(k).name for k in range(0, self._mj_model.njnt)
        ]
        self.backlash_joint_names = [
            j for j in self.joint_names
            if j not in self.actuator_names and j != self.floating_base_name  # Bug fix: equality, not substring
        ]  # backlash-only joints (empty for LEAP hand)
        ...
        all_idx = self.actuator_joint_ids + [self.get_joint_id_from_name(self.floating_base_name)]
        all_idx = sorted(all_idx)  # Bug fix: jp.array().sort() returns a new array and was not assigned; use Python sorted() here
        self.all_joint_no_backlash_ids = all_idx
...
# --- Patch: mjx.make_data() must not pin an explicit device under pmap ---
# mujoco-mjx>=3.3.3's mjx.make_data() calls _resolve_impl_and_device(), which
# -- when the caller passes no device (our case: mjx.make_data(model) inside
# env.reset()) -- concretizes device to jax.devices()[0] and forwards it to a
# final `jax.device_put(d, device=device)` inside _make_data_jax(). Newer jax
# (confirmed broken on 0.10.2) implements pmap via shard_map, which raises
# `ValueError: device_put with explicit device not allowed within
# shard_map-decorated functions` for that call, since env.reset() always runs
# inside jax.pmap(jax.vmap(reset)) during brax training.
# Fix: only when the caller didn't request a specific device, keep it None so
# device placement is deferred to JAX's tracing context instead of being
# pinned explicitly. Verified empirically against mujoco-mjx 3.3.0-3.10.0;
# every version with mjx.Data._impl (>=3.3.3, required by mujoco_playground
# v0.0.5's collision.geoms_colliding) exhibits this, so the workaround must
# stay regardless of which mjx version is pinned.
import mujoco.mjx._src.io as _mjx_io  # noqa: E402

_orig_resolve_impl_and_device = _mjx_io._resolve_impl_and_device


def _resolve_impl_and_device_no_explicit_device(impl=None, device=None):
    resolved_impl, resolved_device = _orig_resolve_impl_and_device(impl, device)
    if device is None:
        return resolved_impl, None
    return resolved_impl, resolved_device


_mjx_io._resolve_impl_and_device = _resolve_impl_and_device_no_explicit_device
# --- End patch ---
